...
$parse = $lang;
	...
	if ($user['authlevel'] >= 3) {
		...
		while ($e = mysql_fetch_array($query)) {
			$i++;
			$parse['msg_list'] .= stripslashes("<tr align='center'><td class=n rowspan=2>{$e['messageid']}</td>" .
			"<td class=n>[<a href=?delete={$e['messageid']}>X</a>]</td>" .
			"<td class=n>{$e['user']}</td>" .
			"<td class=c>" . date('Y-m-d h:i:s', $e['timestamp']) . "</td></tr>" .
			"<tr><td class=b colspan=4>" . nl2br($e['message']) . "</td></tr>");
		}
