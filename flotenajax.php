...
include($ugamela_root_path . 'common.' . $phpEx);
	...
	//+by sbdx 1:20 2008-7-14
	//判断间谍卫星数量
	if( isset($_POST['ship210']) and intval($_POST['ship210'])>0 )
	{
		$ship=intval($_POST['ship210']);
		if($ship>99){$ship=99;}
		$sql="select spy_sonde as spy from {{table}} where id=".$planetrow['id'];
		$rs=doquery($sql,"planets");
		$row=mysql_fetch_array($rs);
		if($row['spy']<$ship)
		{
			die("600;{$lang['gs_notenoughfleet']}|1 0 0 0");
		}
		unset($row,$rs,$sql);
	}
