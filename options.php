...
if ($_POST && $mode == "change") { // Array ( [db_character]
	$iduser = $user["id"];
	$avatar = $_POST["avatar"];
	//* by sbdx 0:25 2008-6-14
	//$dpath = $_POST["dpath"];
	$dpath = (!$_POST["dpath"]) ? DEFAULT_SKINPATH : $_POST["dpath"];
	...
	{
		$urlaubs_modus = "1";//开启
		$urlaubs_modus_time=time()+43200;//12小时
	}
	else
	{
		//对没有checkbox情况下进行判断
	...
	*/
	message($lang['succeful_save'], $lang['Options'],'options.php');
} else {
	$parse = $lang;

	$parse['dpath'] = $dpath;
	$parse['opt_lst_skin_data']  = "<option value ='skins/xnova/'>skins/xnova/</option><option value='skins/evolution/'>skins/evolution/</option>";
	...
	if($user['urlaubs_modus_time']>time())
	{
		$parse['opt_modev_data']="时间截止到:".date("Y-m-d H:i:s",$user['urlaubs_modus_time'])."<br>当前时间:".date("Y-m-d H:i:s",time());
	}
	else
	{
		$opt_modev_data = ($user['urlaubs_modus'] == 1)?"checked='checked'":'';
		$parse['opt_modev_data'] ="<input name='urlaubs_modus'$opt_modev_data type='checkbox' />";
	}
