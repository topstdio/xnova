...
include($ugamela_root_path . 'common.' . $phpEx);
	...
	$protection      = $game_config['noobprotection'];//新手保护
	$protectiontime  = $game_config['noobprotectiontime'];//新手保护时间
	$protectionmulti = $game_config['noobprotectionmulti'];//积分差距倍数
	if ($protectiontime < 1) {
	...
	$VacationMode = $HeDBRec['urlaubs_modus'];	//假期模式
	if($user['authlevel']<1)//新手保护,管理员除外
	{
		if ($MyGameLevel > ($HeGameLevel * $protectionmulti) AND
			$TargetPlanet['id_owner'] != '' AND
			$_POST['mission']     == 1  AND
			$protection           == 1  AND
			$HeGameLevel < ($protectiontime * 500)) {
		...
		if ($MyGameLevel > ($HeGameLevel * $protectionmulti) AND
			...
			$protection           == 1  AND
			$HeGameLevel < ($protectiontime *500)) {
			message("<font color=\"lime\"><b>2.".$lang['fl_noob_mess_n']."</b></font>", $lang['fl_noob_title'], "fleet." . $phpEx, 2);
		}
		...
		if ($MyGameLevel > ($HeGameLevel * $protectionmulti) AND
			...
			$protection           == 1  AND
			$HeGameLevel < ($protectiontime * 500)) {
			message("<font color=\"lime\"><b>3.".$lang['fl_noob_mess_n']."</b></font>", $lang['fl_noob_title'], "fleet." . $phpEx, 2);
		}
		...
		if (($MyGameLevel * $protectionmulti) < $HeGameLevel AND
			...
			$_POST['mission']     == 1  AND
			$protection           == 1  AND
			$MyGameLevel < ($protectiontime * 500)) {
			message("<font color=\"lime\"><b>4.".$lang['fl_noob_mess_n']."</b></font>", $lang['fl_noob_title'], "fleet." . $phpEx, 2);
		}
		...
		if (($MyGameLevel * $protectionmulti) < $HeGameLevel AND
			...
			$protection           == 1  AND
			$MyGameLevel < ($protectiontime * 500)) {
			message("<font color=\"lime\"><b>5.".$lang['fl_noob_mess_n']."</b></font>", $lang['fl_noob_title'], "fleet." . $phpEx, 2);
		}
		...
		if (($MyGameLevel * $protectionmulti) < $HeGameLevel AND
			...
			$protection           == 1  AND
			$MyGameLevel < ($protectiontime * 500)) {
			message("<font color=\"lime\"><b>6.".$lang['fl_noob_mess_n']."</b></font>", $lang['fl_noob_title'], "fleet." . $phpEx, 2);
		}
	}
	...
	if (($user[$resource[108]] + 1) <= $ActualFleets) {
		message($lang['fl_expe_max'], "错误", "fleet." . $phpEx, 1);
	}
...
{
	var dt2 = new Date();
	var curdt = dt2.getTime();
	if(curdt >= ende) {window.location.href="fleet.php";}
}
