				...
				if (!$InResearch) {
					...
					if ($CanBeDone) {
						if (!CheckLabSettingsInQueue ( $CurrentPlanet )) {
							...
							$TechnoLink  = "<button onClick=\"this.disabled=true;location.href='buildings.php?mode=research&cmd=search&tech=".$Tech."';\">";
							if ($LevelToDo == 1) {
								$TechnoLink .= "". $lang['Rechercher'] ."</button>";
							} else {
								$TechnoLink .= "". $lang['Rechercher'] ."<br>".$lang['level']." ".$LevelToDo."</button>";
							}
							$TechnoLink  .= "</a>";
						}
					} else {
						if ($LevelToDo == 1) {//不能建造
							$TechnoLink  = "<button disabled style='color:red'>". $lang['Rechercher'] ."</button>";
						} else {
							$TechnoLink  = "<button  style='color:red'>". $lang['Rechercher'] ."<br>".$lang['level']." ".$LevelToDo."</button>";
						}
					}
