		...
		if (in_array($Element, $Allowed[$CurrentPlanet['planet_type']])) {
			...
			if (IsTechnologieAccessible($CurrentUser, $CurrentPlanet, $Element)) {
				...
				if       ($parse['click'] != '') {
					...
					if ($Queue['lenght'] == 0) {
						if ($NextBuildLevel == 1) {
							if ( $HaveRessources == true ) {
								$parse['click'] = "<button onClick=\"this.disabled=true;location.href='?cmd=insert&building=". $Element ."'\">". $lang['BuildFirstLevel'] ."";
							} else {
								$parse['click'] = "<button style='color:red'>". $lang['BuildFirstLevel'] ."</button>";
							}
						} else {
							if ( $HaveRessources == true ) {
								$parse['click'] = "<button onClick=\"this.disabled=true;location.href='?cmd=insert&building=". $Element ."'\">". $lang['BuildNextLevel'] ."<br>" . $lang['level'] . $NextBuildLevel ."</button>";
							} else {
								$parse['click'] = "<button style='color:red'>". $lang['BuildNextLevel'] ."<br>". $NextBuildLevel ."</button>";
							}
						}
					} else {
						$parse['click'] = "<button onClick=\"this.disabled=true;location.href='?cmd=insert&building=". $Element ."'\">". $lang['InBuildQueue'] ."</button>";
					}
				} elseif ($RoomIsOk && !$CanBuildElement) {
					if ($NextBuildLevel == 1) {
						$parse['click'] = "<button style='color:red'>". $lang['BuildFirstLevel'] ."</button>";
					...
					$parse['click'] = "<button style='color:red'>". $lang['NoMoreSpace'] ."</button>";
				}
