...
function FleetBuildingPage ( &$CurrentPlanet, $CurrentUser ) {
	...
	if (isset($_POST['fmenge'])) {
		...
		// Et y a une liste de dolances
		$AddedInQueue                     = false;
		// Ici, on sait precisement ce qu'on aimerait bien construire ...
		foreach($_POST['fmenge'] as $Element => $Count) {
			// Construction d'Element recupers sur la page de Flotte ...
			// ATTENTION ! La file d'attente Flotte est Commune a celle des Defenses
			// Dans fmenge, on devrait trouver un tableau des elements constructibles et du nombre d'elements souhaits
	...
	$TabIndex = 0;
	$PageTable .='<tr al><td class="c">外观</td><td class="c">描述</td><td class="c">数量</td></tr>';
	foreach($lang['tech'] as $Element => $ElementName) {
		if ($Element > 201 && $Element <= 399) {
			if (IsTechnologieAccessible($CurrentUser, $CurrentPlanet, $Element)) {
				...
				if ($CanBuildOne) {
					$TabIndex++;
					$PageTable .= "<input type=text name=fmenge[".$Element."] alt='".$lang['tech'][$Element]."' size=5 maxlength=6 value=0 tabindex=".$TabIndex.">";
				}
				$PageTable .= "</th>";
