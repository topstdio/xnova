...
function MissionCaseAttack ( $FleetRow ) {
	...
	if ($FleetRow['fleet_start_time'] <= time()) {
		if ($FleetRow['fleet_mess'] == 0) {
			...
			$AttackDate        = date("Y-m-d H:i:s", $FleetRow["fleet_start_time"]);
			...
			foreach ($dane_do_rw as $a => $b)//循环显示每轮攻击
			{
				//攻击者sbdx [1:1:1] 武器:150% 护盾:150% 装甲:150%
				...
				//防御者 xxx [1:1:2] 武器:80% 护盾:60% 装甲:100%
				if ($b["wrog"]['count'] > 0) {
					$raport1 = "<tr><th>".$lang['sys_ship_type']."</th>";
					$raport2 = "<tr><th>".$lang['sys_ship_count']."</th>";
					$raport3 = "<tr><th>".$lang['sys_ship_weapon']."</th>";
					$raport4 = "<tr><th>".$lang['sys_ship_shield']."</th>";
					$raport5 = "<tr><th>".$lang['sys_ship_armour']."</th>";
					foreach ($b["wrog"] as $Ship => $Data)//显示舰队
					{
						if (is_numeric($Ship)) {
							if ($Data['count'] > 0) {
								$raport1 .= "<th>". $lang["tech_rc"][$Ship] ."</th>";
				...
				{
					$zniszczony = true;
					$raport .= "<br />". $lang['sys_destroyed'];//被摧毁了
				}
				$raport .= "</table></center></th></tr></table>";

				if (($zniszczony == false) and !($a == 8))
				{
					//进攻者舰队总共发射强度为 %s 的能量射向防御者。防御者的防御盾吸收了 %s 点伤害.
					//防御者舰队总共射击强度为 %s 的能量射向进攻者。进攻者的防御盾吸收了 %s 点伤害.
